...
define(["dojo/_base/array",
        ...
        "wamc/_MessagesMixin",
        "dojo/text!wamc/widget/wizard/templates/ServiceConfigurationWizard.html",
        ...
        function(array,declare,Deferred,event,json,lang,xhr,aspect,
        		...
        		domStyle,fx,on,
        		MemoryStore,string,topic,Button,Form,RadioButton,ContentPane,
        		registry,Tree,ForestStoreModel,_TemplatedMixin,_Widget,
        		_WidgetsInTemplateMixin,Uploader,Standby,WizardPane,Cache,Grid,
        		Select,TextBox,Resources,gridFormat,gridModules,
        		validation,Wizard,_FormElementIdMixin,UploaderValueTextBox,
        		_HelpLabelsMixin,_MessagesMixin,template,nls){
	
	var ServiceConfigurationWizard = declare("wamc.widget.wizard.ServiceConfigurationWizard",
		...
		postCreate:function(){
			...
			var deploymentPolicyGrid = this.deploymentPolicyGrid = new Grid({
				...
				structure:[
				           { id: "applianceName", name: nls.serviceConfigurationWizard.deploymentPolicy.grid.applianceName, field:"applianceName", width:"30%"},
				           { id: "name", name: nls.serviceConfigurationWizard.deploymentPolicy.grid.name, field:"name", width:"30%"},
				           { id: "policy", name: nls.serviceConfigurationWizard.deploymentPolicy.grid.policy, field: "deploymentPolicy",width:"40%",formatter:gridFormat.formatDeploymentPolicy}],
				"aria-label":nls.serviceConfigurationWizard.deploymentPolicy.gridHeader
				});
		...
		_configSourceUploadComplete:function(/*Object*/result){
			...
			// result: Object
			//		Object containing the list of files returned by the server
			// tags:
			//		protected
			...
			Deferred.when(this._configSourceGetServiceList(applianceIds, uri),
					lang.hitch(this,function(services){
						...
						// Does the list have appropriate config?
						success = array.some(services,function(item){
							return item.name === this.model.name &&
						...
						if(success){
							...
							this.showMessage("ERROR_NO_MATCHING_SERVICES",{name:this.model.name});
						}
		...
		_configSourceGetServiceList:function(/*String*/applianceId,/*String*/uri){
			// summary:
			...
			console.debug(F,applianceId,uri);
		...
		_serviceImpactShowNoImpact:function(){
			...
			domAttr.set(this.serviceImpactNoImpactNode,"innerHTML",nls.serviceConfigurationWizard.serviceImpact.noImpact);
		},
			...
			var newImpacts = array.map(data,function(impact){
				...
				ni.files = array.map(impact.files,function(file){
					return {
						id:i++,
						type:"file",
						name:file + " (" + nls.serviceConfigurationWizard.serviceImpact.file + ")"
					};
				},this);
		...
		_serviceImpactShowWarning:function(/*String*/serviceName){
			...
			var msg = string.substitute(nls.serviceConfigurationWizard.serviceImpact.warning,{name:serviceName});
		...
		serviceImpactOnNodeMouseEnter: function(/*dijit._Widget*/ node){
			...
			if(type==="service"){
				var tooltip = string.substitute(nls.serviceConfigurationWizard.serviceImpact.serviceTooltip,{serviceName:item.serviceName,serviceType:item.serviceType});
				tooltip += "<ul>";
				array.forEach(item.deployments,function(deployment){
					tooltip += "<li>";
					tooltip += string.substitute(nls.serviceConfigurationWizard.serviceImpact.serviceTooltipListItem,{appliance:deployment.applianceName,domain:deployment.domainName});
				});
				tooltip += "</ul>";
	...
	return ServiceConfigurationWizard;
});

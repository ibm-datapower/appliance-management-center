...
define(["dojo/_base/array",
        ...
        "dojo/text!wamc/widget/wizard/templates/CreateDomainWizard.html"],
        function(array,declare,Deferred,event,lang,xhr,aspect,domClass,
        		domConstruct,domStyle,fx,on,string,topic,MemoryStore,
        		_TemplatedMixin,_Widget,_WidgetsInTemplateMixin,Button,Form,
        		RadioButton,registry,Uploader,Standby,Grid,Cache,Select,
        		TextBox,_HelpLabelsMixin,_MessagesMixin,Resources,
        		gridFormat,gridModules,Wizard,_FormElementIdMixin,
        		UploaderValueTextBox,wamcValidation,UploadStandby,nls,template){
        
	var CreateDomainWizard = declare("wamc.widget.wizard.CreateDomainWizard",
		...
		postCreate:function(){
			...
			console.debug(F, "Setup Widgets: createDomainPane");
			
			var createDomainGrid = this.createDomainGrid = new Grid({
				store: new MemoryStore({data:null}),
				cacheClass: Cache,
				structure:[
				           { id: "name", name: nls.createDomainWizard.createDomain.grid.name, field: "name", width:"50%" },			        
				           { id: "applianceType", name: nls.createDomainWizard.createDomain.grid.applianceType, field: "applianceType",width:"50%" }],
				modules:[{moduleClass:gridModules.Focus}],
				'aria-labelledby': this.id +"_createDomain_summaryHeader"
				});
			createDomainGrid.placeAt(this.createDomainGridContainerNode);
			
			domClass.add(createDomainGrid.domNode,"compact");
			
			createDomainGrid.startup();
			...
			var deploymentPolicyGrid = this.deploymentPolicyGrid = new Grid({
				...
				structure:[
				           { id: "name", name: nls.createDomainWizard.deploymentPolicy.grid.name, field: "name",width:"50%"},			        
				           { id: "policy", name: nls.createDomainWizard.deploymentPolicy.grid.policy, field: "deploymentPolicy",width:"50%",formatter:gridFormat.formatDeploymentPolicy}],
				'aria-label':nls.createDomainWizard.deploymentPolicy.gridHeader
				});
			...
			console.debug(F, "Setup Events: CreateDomainPane");
			
			this.createDomainForm.watch("state",function(property,oldValue,newValue){
			...
			// Validation for Create Domain Panel
			this.createDomainPane.set("passFunction",
					lang.hitch(this,"createDomainPassFunction"));

			this.createDomainPane.set("actionFunction",
					lang.hitch(this,"createDomainActionFunction"));
		...
		_updateDisplay:function(/*Object*/model){
			...
			/** Create Domain Pane **/
			this._resetGrid(this.createDomainGrid,new MemoryStore({data:model.appliances,idProperty:"name"}));
			this.createDomainDomainNameDataField.set("value",model.name);
			...
			this._setupGridSelectionEvents(this.deploymentPolicyGrid);
		...
		_updateModel:function(){
			...
			/** Create Domain Pane **/
			this.model.name = this.createDomainDomainNameDataField.get("value");
		...
		passFunction:function(){
			...
			return (this.createDomainPassFunction()===true && 
			...
			if(source==="local"){
				...
				this.uploadStart();
				on(this.configSourceLocalSourceUploader, "progress", lang.hitch(this, function(evt) {
					var pct = (Math.floor(100*evt.bytesLoaded/evt.bytesTotal));
					this.uploadStandby.setValue(pct);
				}));
		...
		uploadStart:function(){
			...
			this.disable();
			this.uploadStandby.show();
			this.uploadStandby.setupProgressBar();
		},
		...
		reset:function(){
			...
			/** Create Domain Form **/
			this._resetGrid(this.createDomainGrid,new MemoryStore({data:[]}));
			this.createDomainDomainNameDataField.reset();
			...
			this.wizard.selectChild(this.createDomainPane);
		},
		...
		resize:function(){
			...
			this._resetGrid(this.createDomainGrid);
			this._resetGrid(this.deploymentPolicyGrid);
		},
		...
		disable: function(){
			...
			// Disable forms
			this.createDomainDomainNameDataField.set("disabled",true);
			this.configSourceRemoteSourceDataField.set("disabled",true);
		...
		enable: function(){
			...
			this.createDomainDomainNameDataField.set("disabled",false);
		...
		/** Functions for CreateDomainPane **/
		
		createDomainPassFunction: function(){
			// summary:
			//		Validation function for the createDomain pane
			// return: Boolean
			//		True if the user can continue, otherwise false
			var F = this.declaredClass + ".createDomainPassFunction()";
			console.debug(F);
			
			if (this.createDomainForm.isValid()) {
		...
		createDomainActionFunction: function(){
			// summary:
			//		Update the model and retrieve domains with the same name
			var F = this.declaredClass + ".createDomainActionFunction()";
			console.debug(F);
			
			// Update the model with the domain name
			var n = this.model.name = this.createDomainDomainNameDataField.get("value");
			
			this.showStandby(nls.createDomainWizard.standbyWorking);
			
			// Retrieve all the domains with the same name,
			// then call function to populate domains list on completion
			var d1 = this.store.query({name:n}),
				d2 = Deferred.when(d1,
						lang.hitch(this,"_createDomainPopulateConfigSource"),
						lang.hitch(this,"actionFunctionError"));
			
			return d2;
		},
		
		_createDomainPopulateConfigSource:function(domains){
			// tags:
			//		protected
			var F = this.declaredClass + "._createDomainPopulateConfigSource()";
			...
			options.push({label: nls.createDomainWizard.configurationSource.nullDomain, value: ""});
			
			array.forEach(domains,function(domain){
				var l = string.substitute(nls.createDomainWizard.configurationSource.existingDomainOption,{applianceName:domain.applianceName,domainName:domain.displayName}),
		...
		_deploymentPolicyPopulateExistingAppliance:function(domains){
			...
			options.push({label: nls.createDomainWizard.deploymentPolicy.nullAppliance, value: ""});
		...
		_deploymentPolicyPopulateExistingDomain:function(applianceId){
			...
			options.push({label: nls.createDomainWizard.deploymentPolicy.nullDomain, value: ""});

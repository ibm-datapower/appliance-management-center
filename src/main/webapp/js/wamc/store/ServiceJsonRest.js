...
define(["dojo/_base/declare",
	...
	var WAMCJsonRestStore = declare("wamc/store/ServiceJsonRest",[JsonRest],{
		...
		remove: function(id,objectsToDelete){
			// summary:
			//		Deletes a service by its identity. This will trigger a 
			//		DELETE request to the server.
			// id: Number
			//		The identity to use to delete the object
			// objectsToDelete:Object
			//		The related objects to be deleted along with this service
			return xhr("DELETE",{
				url:this.uriFromPrimaryKey(id,this.instanceTarget),
				postData: JSON.stringify(objectsToDelete),
				handleAs: "json",
				headers: {
					"Content-Type": "application/json",
		...
		uriFromPrimaryKey:function(/*String*/primaryKey,/*String*/resource){
			// summary:
			//		Build the URI for a service from its unique primaryKey
			// primaryKey: String
			//		The primaryKey for the service
			// resource: String
			//		The template string for the resource that will be called,
			//		containing 'applianceId' and 'domainName', 'name' and 'className' tokens
			// return: String
			//		The template
			...
			if (parts.length < 4){
				...
				return string.substitute(resource,{'applianceId':parts[0],'domainName':parts[1],'name':parts[2],'className':parts[3]});
			}
		}
